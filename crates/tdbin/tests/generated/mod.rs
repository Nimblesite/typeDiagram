...
//! from a fixed Person-shaped `.td`. `roundtrip.rs` exercises these types so the
//! end-to-end proof — typeDiagram ADT -> binary -> typeDiagram ADT — runs under
//! `cargo test`. The `rust-tdbin.test.ts` vitest suite pins the emitted text and
//! fails if this file drifts from codegen; regenerate rather than hand-editing.
